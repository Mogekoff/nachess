...
class Figure:       # базовый класс "фигура"
    ...
    def move(self, x, y):   # функция, делающая ход
        if self.can_move(x, y):             # проверяем, можем ли сходить
            self.field[x][y] = self         # если да, ходим
            self.field[self.x][self.y] = None   # удаляем себя из прошлой клетки
            self.x = x      # меняем свои координаты
            self.y = y      # на новые
...
class Pawn(Figure):     # класс пешки
    ...
    def move(self, x, y):
        if self.can_move(x, y):     # проверяем, можем ли сходить
            super().move(x, y)      # ходим через базовый метод родительского класса

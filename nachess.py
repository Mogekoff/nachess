...
class Figure:
    ...
    def can_move(self, x, y):
        if x < 0 or x > 7 or y < 0 or y > 7 or self.x == x and self.y == y:
            return False
        return True
    ...
    def can_move(self, x, y):
        if super().can_move(x, y):
            if ((self.x == x and
                ...
                ((self.x-1 == x or self.x-1) and field.field[x][y] is not None and not self.color == field.field[x][y].color)
                ):
                self.move(x, y)
...
field.field[1][1].can_move(1,2)
field.field[2][1].can_move(2,3)
field.field[3][1].can_move(3,4)
field.field[4][1].can_move(4,1)
field.field[4][1].can_move(5,0)
field.print()
